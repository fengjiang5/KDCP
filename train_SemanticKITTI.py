...
def main(args):
    ...
    my_BEV_model=BEV_Unet(n_class=len(unique_label)+1, n_height = compression_model, 
                            input_batch_norm = True, dropout = 0.5, 
    ...
    if os.path.exists('pretrained_model/cylinder3d/model_save_backup.pt'):
        print('Load Cylinder3D ing...')
        cylinder_model.load_state_dict(torch.load('pretrained_model/cylinder3d/model_save_backup.pt'))
    ...
    while epoch < 40:
        ...
        for i_iter,(_,train_vox_label,train_grid,train_pt_labs,train_pt_fea,label_path) in enumerate(train_dataset_loader):
            ...
            loss = lovasz_softmax(torch.nn.functional.softmax(outputs), point_label_tensor,ignore=0) + loss_fun(outputs,point_label_tensor)
            
            loss_kd = loss_func_kd(kd_cylinder[0], kd_polar[0]) +loss_func_kd(kd_cylinder[1], kd_polar[1])+\
